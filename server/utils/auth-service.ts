import { Prisma, UserStatus, type UserProfile } from "@prisma/client";
...
async function ensureBootstrapAdmin(profile: UserProfile) {
  if (profile.isBootstrapAdmin || !profile.account) {
    return profile;
  }

  const existingBootstrap = await prisma.userProfile.findFirst({
  ...
  const firstAccount = await prisma.userProfile.findFirst({
    where: { account: { not: null } },
    orderBy: [{ createdAt: "asc" }, { id: "asc" }],
    select: { id: true }
  });
  if (firstAccount?.id !== profile.id) {
    return profile;
  }
...
export async function syncAdminSnapshot(profile: UserProfile) {
  const bootstrapped = await ensureBootstrapAdmin(profile);
  const nextIsAdmin =
    bootstrapped.isBootstrapAdmin ||
    isAdminAccount(bootstrapped.account) ||
    isAdminEmail(bootstrapped.email);

  if (bootstrapped.isAdminSnapshot === nextIsAdmin) {
    return bootstrapped;
  }
...
export async function registerAccountUser(params: {
  ...
  return prisma.$transaction(
    async (tx) => {
      const exists = await tx.userProfile.findUnique({ where: { account } });
      if (exists) {
        throw createError({ statusCode: 409, message: "该账号已注册" });
      }

      const accountUserCount = await tx.userProfile.count({
        where: { account: { not: null } }
      });
      const isBootstrapAdmin = accountUserCount === 0;
      ...
      return tx.userProfile.create({
        data: {
          ...
          isBootstrapAdmin,
          isAdminSnapshot: isAdmin || isBootstrapAdmin,
