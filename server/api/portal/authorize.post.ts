...
import { normalizeUrl, sameUrlWithoutSearch } from "~/server/utils/crypto";
...
export default defineEventHandler(async (event) => {
  ...
  if (sameUrlWithoutSearch(callbackUrl, service.homeUrl)) {
    return {
      status: "authorized",
      redirectUrl: normalizeUrl(service.homeUrl).toString(),
      expiresIn: 0
    };
  }
