...
export default defineEventHandler(async (event) => {
  ...
  const rawMaxUses = Number(body.maxUses ?? 1);
  if (!Number.isInteger(rawMaxUses) || rawMaxUses < 1) {
    throw createError({ statusCode: 400, statusMessage: "可用次数必须是大于 0 的整数" });
  }
  ...
  const quantity = generationMode === "batch" ? rawQuantity : 1;
  ...
  if (expiresAt && expiresAt.getTime() <= Date.now()) {
    throw createError({ statusCode: 400, statusMessage: "过期时间必须晚于当前时间" });
  }
