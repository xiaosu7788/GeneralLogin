...
export default defineEventHandler(async (event) => {
  ...
  if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(slug)) {
    throw createError({ statusCode: 400, statusMessage: "Slug 只能包含小写字母、数字和中划线" });
  }

  const existingService = await prisma.serviceApp.findUnique({
    where: { slug },
    select: { id: true }
  });
  if (existingService) {
    throw createError({ statusCode: 409, statusMessage: "Slug 已被其他服务使用，请更换" });
  }
